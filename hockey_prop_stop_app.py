# ---------------------------------------------------------------
# 🏒 Hockey Prop Stop — L5 Probability Update + Save + Dashboard
# ---------------------------------------------------------------
...
with tab1:
    ...
    if "results_raw" in st.session_state and not st.session_state.results_raw.empty:
        ...
        div.scrollable-table {{
            overflow-x:auto;overflow-y:auto;height:600px;position:relative;
        }}table {{
            width:100%;border-collapse:collapse;font-family:'Source Sans Pro',sans-serif;color:#f0f0f0;
        }}th {{
            background-color:#00B140;color:white;padding:6px;text-align:center;position:sticky;top:0;z-index:3;
        }}td:first-child,th:first-child {{
            position:sticky;left:0;z-index:4;background-color:#00B140;color:white;font-weight:bold;
        }}td {{
        ...
        if st.button("💾 Save Projections for Selected Date"):
            ...
            if os.path.exists(master):
                ...
                pd.concat([existing, out_df], ignore_index=True).to_csv(master, index=False)
            else:
                out_df.to_csv(master, index=False)
            matchup_file = f"projections/{team_a}_vs_{team_b}_{out_df['Date_Game'].iloc[0]}.csv"
            out_df.to_csv(matchup_file, index=False)
            st.success(f"✅ Saved projections for {selected_date} to both {master} and {matchup_file}")
    ...
    if not os.path.exists(master):
        ...
        if actuals_file:
            ...
            if player_col and sog_col:
                ...
                st.metric("Mean Absolute Error (MAE)", f"{ma
