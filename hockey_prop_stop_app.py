# ---------------------------------------------------------------
# 🏒 Hockey Prop Stop — Fast Version with Cached Model + Trends
# ---------------------------------------------------------------
...
st.markdown(
    ...
    <p style='text-align:center;color:#BFC0C0;'>
        Team-vs-Team matchup analytics with regression + trend visualization
    </p>
    """,
...
# ---------------------------------------------------------------
# Cached Model (runs once per matchup)
# ---------------------------------------------------------------
@st.cache_data(show_spinner=True)
def build_model(team_a, team_b, skaters_df, shots_df, goalies_df, lines_df):
    # Use your existing working model loop code here (unchanged)
    # Just ensure you return df at the end.
    # This is placeholder to simulate your existing logic.
    results = []
    # ... your full working model logic from before ...
...
if st.session_state.results is not None and not st.session_state.results.empty:
    ...
    # ---------------------------------------------------------------
    # Player Trend Visualization (fast dropdown)
    # ---------------------------------------------------------------
    st.markdown("### 📈 Player Regression Trend Viewer")
    player_list = st.session_state.results["Player"].unique().tolist()
    selected_player = st.selectbox("Select a player to view detailed trend:", player_list, key="player_dropdown")

    # Filter without triggering model rebuild
