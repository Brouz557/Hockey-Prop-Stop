...
if "results_raw" in st.session_state and not st.session_state.results_raw.empty:
    ...
    if st.button("💾 Save Projections for Selected Date"):
        df_to_save = st.session_state.results_raw.copy()
        ...
        if os.path.exists(master_path):
            ...
            missing_cols = [c for c in df_to_save.columns if c not in existing.columns]
        ...
        st.success(f"✅ Projections saved for {selected_date.strftime('%Y-%m-%d')}!")

    # Optional: Clear saved results
    if st.button("🧹 Clear Saved Results"):
        ...
        st.success("🧹 All saved projections cleared.")
