...
if run_model:
    ...
    if not df.empty and "Projected Goals" in df and "Shooting %" in df:
        df["Goal Signal"] = (
            (df["Projected Goals"] / df["Projected Goals"].max()) * 0.6 +
            (df["Shooting %"] / df["Shooting %"].max()) * 0.4
        ).fillna(0)

        df["Scoring Tier"] = pd.qcut(
            df["Goal Signal"].rank(method="first"), 4,
...
if st.session_state.results is not None:
    ...
    sortable_df = st.session_state.results[display_cols]
    ...
    chart = alt.Chart(st.session_state.results).mark_circle(size=80).encode(
        ...
        tooltip=[c for c in ["Player", "Team", "Projected Goals", "Shooting %",
                             "Scoring Tier", "Final Projection"] if c in st.session_state.results.columns]
    ).interactive()
