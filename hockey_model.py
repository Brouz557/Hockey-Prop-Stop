# ---------------------------------------------------------------
# hockey_model.py
# Hockey Prop Stop – stable, matchup-aware analytics engine
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# Safe, fast parser with auto-sampling for recent games
# ---------------------------------------------------------------
def parse_raw_files(file_dfs):
    ...
    for name, df in file_dfs.items():
        try:
            ...
            # Keep at most 100k rows to avoid Streamlit lockups
            if len(df) > 100000:
                df = df.tail(100000)
    ...
    if shots.empty:
        ...
        # keep only most recent 3000 game rows
        if len(shots) > 3000:
...
# ---------------------------------------------------------------
# Build matchup model with exponential weighting
# ---------------------------------------------------------------
def build_matchup_model(skaters, teams, shots, goalies, lines, team_a, team_b):
    """Build regression for two selected teams using rolling & trend features."""
    df = shots.copy()
