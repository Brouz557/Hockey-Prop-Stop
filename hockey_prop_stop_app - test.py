...
def build_model(team_a, team_b, skaters_df, shots_df, goalies_df, lines_df, teams_df, injuries_df):
    ...
    for row in roster.itertuples(index=False):
        ...
        form_flag="🟢 Above Baseline" if trend>0.05 else \
                  "🔴 Below Baseline" if trend<-0.05 else "⚪ Neutral"

        line_factor_internal=1.0
        if isinstance(line_adj,pd.DataFrame) and not line_adj.empty:
            last_name=str(player).split()[-1].lower()
            m=line_adj[line_adj["line pairings"].str.contains(
        ...
        poisson_prob=float(
            np.clip(1-poisson.cdf(np.floor(lam)-1,mu=max(lam,0.01)),0.0001,0.9999)
        )

        odds=-100*(poisson_prob/(1-poisson_prob)) if poisson_prob>=0.5 \
             else 100*((1-poisson_prob)/poisson_prob)
        ...
        if not injuries_df.empty and {"player","team"}.issubset(injuries_df.columns):
            ...
            if not match.empty:
                ...
                tooltip="\n".join([p for p in [injury_type,note,date_injury] if p])
                safe=html.escape(tooltip)
                injury_html=f"<span onclick='alert({json.dumps(safe)})'>🚑</span>"

        if "goal" in df_p.columns:
            agg=df_p.groupby(game_col).agg({"sog":"sum","goal":"sum"}).reset_index()
            shots_pg=agg["sog"].mean()
            goals_pg=agg["goal"].mean()
            shooting_pct=goals_pg/shots_pg if shots_pg>0 else 0
    ...
    for i,m in enumerate(games):
        ...
        with cols[i%3]:
            with st.form(f"form_{i}"):
                st.markdown(f"""
                <div style="background-color:#1C5FAF;color:#fff;
