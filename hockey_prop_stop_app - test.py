...
def run_analysis():
    ...
    if uploaded_skaters and uploaded_shots:
        df = pd.read_csv(uploaded_skaters)
...
ranked = data.sort_values("Probability (Over)", ascending=False).reset_index(drop=True)
...
with col1:
    ...
    sns.scatterplot(x="Projected SOG", y="Probability (Over)", data=ranked, hue="Signal Strength", s=100)
