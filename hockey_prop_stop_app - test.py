...
def build_model(team_a, team_b, skaters_df, shots_df, goalies_df, lines_df, teams_df, injuries_df):
    ...
    for row in roster.itertuples(index=False):
        ...
        lam=baseline*(1+(goalie_factor-1)*0.2)
...
if run_model:
    ...
    for m in games:
        df=build_model(
            m["away"],m["home"],
            skaters_df,shots_df,goalies_df,lines_df,teams_df,injuries_df
        )
        if not df.empty:
...
# ---------------------------------------------------------------
# Display Results + LINE TO TEST ABOVE TABLE
# ---------------------------------------------------------------
if "results" in st.session_state:
    df=st.session_state.results.copy()

    st.markdown("### 🎯 Shot Line Test")
    st.session_state.line_test_val = st.number_input(
        "Line to Test",
        0.0,10.0,
